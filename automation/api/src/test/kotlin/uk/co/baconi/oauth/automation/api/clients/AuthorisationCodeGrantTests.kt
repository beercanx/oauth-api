...
import uk.co.baconi.oauth.automation.api.driver.RestAssuredDriver
...
class AuthorisationCodeGrantTests : RestAssuredDriverTest() {
    ...
    fun `authorisation code grant`(client: ConfidentialClient, user: User) {
        ...
        driver.authorise(client, state, scopes, expectedStatus = 200)

        val csrfToken = driver.getAuthenticationCsrfToken()
    ...
    fun `authorisation code grant using PKCE`(client: Client, user: User) {
        ...
        driver.authorise(client, state, scopes, codeChallenge, codeChallengeMethod, 200)
    ...
    private fun RestAssuredDriver.getAuthenticationCsrfToken(): String {
        return given(browserSpecification)
            .accept(ContentType.JSON)
            .baseUri(authenticationLocation.toASCIIString())
            .get("/session")
            .then()
            .statusCode(200)
            .body("csrfToken", not(blankOrNullString()))
            .extract()
            .jsonPath()
            .getString("csrfToken")
    }
